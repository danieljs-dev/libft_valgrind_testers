...
int main(int argc, char *argv[]) {
    ...
    const char test_cases[] = {
        ...
        'A', 'B', 'C', '1', '!', ' ', // Letras maiúsculas, números e caracteres especiais
        '\0' // Caractere nulo
    };
    ...
    printf("\033[1;34m%-5s %-15s %-15s %-15s\033[0m\n", 
           "C", "ft_toupper", "toupper", "Match");

    for (int i = 0; i < num_tests; i++) {
        int c = test_cases[i];
        int expected = toupper(c); // Usando a função original
        int result = ft_toupper(c); // Usando sua função
